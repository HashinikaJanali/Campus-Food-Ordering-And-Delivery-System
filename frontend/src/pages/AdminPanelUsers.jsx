...
const RoleBadge = ({ role }) => (
  <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest
    ${role === "admin" ? "bg-orange-100 text-primary border border-orange-200" : "bg-blue-50 text-blue-600 border border-blue-200"}`}>
    {role === "admin" ? <ShieldCheck size={11} /> : <GraduationCap size={11} />} {role}
  </span>
);
          ...
          <AnimatePresence>
            {filtered.map((user, i) => (
              <motion.div key={user._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: i * 0.03 }}
                ...
                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center font-black text-sm flex-shrink-0 ${user.role === "admin" ? "bg-orange-100 text-primary" : "bg-blue-100 text-blue-600"}`}>
