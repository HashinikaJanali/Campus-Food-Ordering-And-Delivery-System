...
import { ClipboardList, History, Search, ChefHat, ChevronDown, Settings, HelpCircle, MapPin, LayoutDashboard, UtensilsCrossed, Package, Tag, Bell, Store, Eye, BarChart3, Edit3, Gift, Ticket, X, ExternalLink, Menu } from 'lucide-react';
...
const inventoryMenuItems = [
  ...
  { path: '/admin/student-menu', icon: ExternalLink, label: 'Student Menu', isExternal: true },
];
...
export default function AdminSidebar({ isCollapsed, setIsCollapsed }) {
  ...
  return (
    <aside className={`bg-white border-r border-gray-200 h-[calc(100vh-80px)] fixed left-0 top-20 shadow-sm hidden md:flex flex-col z-40 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-80'}`}>
      {/* Logo Section */}
      <div className={`pt-10 pb-6 px-6 border-b border-gray-100 flex items-center gap-3 bg-gray-50/30 transition-all ${isCollapsed ? 'justify-center' : ''}`}>
        <div className="w-10 h-10 bg-gradient-to-br from-orange-400 to-orange-600 rounded-xl flex items-center justify-center shadow-lg shadow-orange-100 shrink-0">
          <ChefHat size={22} className="text-white" />
        </div>
        {!isCollapsed && (
          <div className="flex-1 min-w-0">
            <h1 className="font-display font-black text-gray-400 text-lg leading-tight tracking-tight uppercase truncate">Grab & Go</h1>
      ...
      <div className="px-4 py-3 border-b border-gray-100">
        <div 
          className={`flex items-center gap-2 rounded-lg transition-all duration-300 border-2 ${isCollapsed ? 'p-2 justify-center' : 'px-3 py-2.5'} ${
            isSearchFocused 
      ...
      <nav className="flex-1 overflow-y-auto custom-scrollbar">
        {!hasResults ? (
          ...
          <>
            {/* Groups */}
            <div className={`py-2 ${isCollapsed ? 'px-2' : ''}`}>
              {!isCollapsed && (
              ...
              {(!searchQuery || 'main dashboard'.includes(searchQuery.toLowerCase())) && (
                <div className={`px-3 mb-2 flex justify-center ${isCollapsed ? 'px-0' : ''}`}>
                  <NavLink
                    to="/admin/management"
                    className={({ isActive }) => `
                    ...
                    `}
                    title={isCollapsed ? "Main Dashboard" : ""}
                  >
                ...
                {(filteredOrderItems.length > 0 || !searchQuery) && (
                  <>
                    <button
                      ...
                      className={`flex items-center gap-3 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-all duration-200 ${
                        isCollapsed ? 'w-10 h-10 justify-center mx-auto' : 'w-full px-3 py-2.5'
                      }`}
                      title={isCollapsed ? "Order Management" : ""}
                    >
                      <ClipboardList size={18} className="shrink-0" />
                      {!isCollapsed && <span className="flex-1 text-left">Order Management</span>}
                      {!isCollapsed && (
                        <ChevronDown 
                          size={16}
                    ...
                    {(!isCollapsed && expandedSections.includes('order-management')) && (
                ...
                {(filteredInventoryItems.length > 0 || !searchQuery) && (
                  <>
                    <button
                      ...
                      title={isCollapsed ? "Inventory Management" : ""}
                    >
                      <Package size={18} className="shrink-0" />
                      {!isCollapsed && <span className="flex-1 text-left">Inventory Management</span>}
                      {!isCollapsed && (
                        <ChevronDown 
                          size={16}
                          className={`shrink-0 transition-transform duration-200 ${
                    ...
                    {(!isCollapsed && expandedSections.includes('inventory-management')) && (
                      <div className="space-y-1 pl-3">
                ...
                {(filteredPromotionsItems.length > 0 || !searchQuery) && (
                  <>
                    <button
                      ...
                      title={isCollapsed ? "Promotion Management" : ""}
                    >
                      <Gift size={18} className="shrink-0" />
                      {!isCollapsed && <span className="flex-1 text-left">Promotion Management</span>}
                      {!isCollapsed && (
                        <ChevronDown 
                          size={16}
                          className={`shrink-0 transition-transform duration-200 ${
                            expandedSections.includes('promotions-management') ? 'rotate-180' : ''
                          }`}
                        />
                      )}
                    </button>
                    ...
                    {(!isCollapsed && expandedSections.includes('promotions-management')) && (
                      <div className="space-y-1 pl-3">
                  ...
                  {filteredOtherItems.map(({ path, icon: Icon, label }) => (
                    <NavLink
                      ...
                      className={({ isActive }) => `
                        flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200
                        ${isActive
                          ? 'bg-gray-100 text-gray-900 shadow-sm border-l-2 border-gray-900 rounded-l-none'
                          : 'text-gray-700 hover:bg-gray-50'
                        }
                      `}
                    >
      ...
      {/* User Info Section (Static for now, matches AdminHeader) */}
      <div className="p-4 border-t border-gray-100 bg-gray-50/50">
        <div className="flex items-center gap-3 px-3 py-2">
          ...
          <div className="flex-1 text-left min-w-0">
            <p className="text-sm font-black text-gray-900 truncate uppercase mt-1 leading-none">Admin</p>
            <p className="text-[10px] font-bold text-orange-500 uppercase tracking-widest truncate mt-1.5">System Admin</p>
          </div>
        </div>
      </div>
    </aside>
